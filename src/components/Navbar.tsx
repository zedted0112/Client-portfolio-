    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <a href="#hero" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-sm bg-[#1a1e27] border border-[#c5a880]/40 flex items-center justify-center text-[#c5a880] font-serif-title font-bold text-xl group-hover:border-[#c5a880] transition-colors">
            NG
          </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <a
                key={item.href}
                href={item.href}
                className={`px-3 py-1.5 text-xs font-sans-body tracking-wide transition-all duration-200 rounded-xs ${
                  isActive
                    ? 'text-[#c5a880] font-semibold bg-[#1a1e27] border-b-2 border-[#c5a880]'
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-x-0 top-[60px] bg-[#0e1116] border-b border-[#262b38] shadow-2xl p-6 backdrop-blur-xl animate-in slide-in-from-top duration-300">
